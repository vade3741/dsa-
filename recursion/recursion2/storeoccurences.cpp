...
int stooccurence(int *a,int i,int n,int key,int *out,int j){
    if(i==n){
        return j;
    }//stopping condition of a for looop
    if(a[i]==key){
        out[j]=i;
      return  stooccurence(a,i+1,n,key,out,j+1);
    }//if condition of a for loop
    //incrementing of i in for loop  
    return stooccurence(a,i+1,n,key,out,j);
}
int main()
{
    int n;
    ...
    int c=firstocc(arr,n,key);
     ...
     int out[n];
     int count=stooccurence(arr,0,n,key,out,0);
